...
def get_vlnbert_models(args, config=None):
    ...
    if args.vlnbert == 'oscar':
        ...
        vis_config = config_class.from_pretrained('bert-base-uncased', local_files_only=True)
        vis_config.img_feature_dim = 2176
        vis_config.img_feature_type = ""
        vis_config.vl_layers = 4
        vis_config.la_layers = 9

        visual_model = model_class.from_pretrained(model_name_or_path, config=vis_config, local_files_only=True)
